    ...
    // std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); 
        ...
        if (StudentAccount::isValidTuition(tuitionInput, tuitionAmount)) {
            // Get total already paid from TXT
            int previousPaid = 0;
            std::string fname = StudentAccount::findFileByPaycode(student.paycode);
            if (!fname.empty()) {
                StudentAccount temp("", "", student.paycode, 0);
                previousPaid = temp.getTotalFromTXT();
            }

            if (tuitionAmount + previousPaid > MAX_TUITION) {
                std::cout << "Payment exceeds remaining tuition balance! "
                        << "You can only pay up to "
                        << (MAX_TUITION - previousPaid) << " UGX.\n";
                continue; // ask again
            }

            break; // valid and within remaining balance
        } else {
